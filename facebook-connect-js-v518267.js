...
if (!window.FB)
	window.FB = {
		...
		_userStatus : 'unknown',
		_logging : true,
		_inCanvas : ((window.name.indexOf('iframe_canvas') > -1) || (window.name
...
FB.provide('Array', {
	...
	forEach : function(a, b, c) {
		...
		if (Object.prototype.toString.apply(a) === '[object Array]'
				|| (!(a instanceof Function) && typeof a.length == 'number')) {
			if (a.forEach) {
				a.forEach(b);
			} else
					...
					insertIframe : function(a) {
						...
						if (document.attachEvent) {
							var e = ('<iframe'
									...
									+ ' onload="FB.Content._callbacks.' + b
									+ '()"' + '></iframe>');
							a.root.innerHTML = '<iframe src="javascript:false"'
									+ ' frameborder="0"' + ' scrolling="no"'
				...
				'Flash',
				{
					...
					init : function() {
						...
						window.FB_OnFlashXdCommReady = function() {
							...
							for ( var a = 0, b = FB.Flash._callbacks.length; a < b; a++)
								FB.Flash._callbacks[a]();
							FB.Flash._callbacks = [];
						};
						FB.Flash.embedSWF('XdComm', FB.getDomain('cdn_foreign')
								+ FB.Flash._swfPath);
					},
					embedSWF : function(a, b, c) {
						var d = !!document.attachEvent, e = ('<object '
						...
						if (FB.UA.ie() >= 9) {
							if (!FB.Flash._unloadRegistered) {
								var f = function() {
									FB.Array.forEach(FB.Flash._names, function(
											g, h) {
					...
					hasMinVersion : function() {
						if (typeof FB.Flash._hasMinVersion === 'undefined') {
							...
							if (a) {
								var h = a.replace(/\D+/g, ',').match(
										/^,?(.+),?$/)[1].split(',');
								for (b = 0, c = h.length; b < c; b++)
									d.push(parseInt(h[b], 10));
							}
							FB.Flash._hasMinVersion = false;
							majorVersion: for (b = 0,
									c = FB.Flash._minVersions.length; b < c; b++) {
								var i = FB.Flash._minVersions[b];
								if (i[0] != d[0])
									continue;
								for ( var j = 1, k = i.length, l = d.length; (j < k && j < l); j++)
									if (d[j] < i[j]) {
										FB.Flash._hasMinVersion = false;
...
FB
		.provide(
				'ApiServer',
				{
					...
					graph : function() {
						var a = Array.prototype.slice.call(arguments), b = a
								.shift().match(/\/?([^?]*)\??([^#]*)/), c = b[1], d = a
								.shift(), e, f, g;
						while (d) {
							var h = typeof d;
							if (h === 'string' && !e) {
								...
								FB.log('Invalid argument passed to FB.api(): '
										+ d);
								return;
							}
							d = a.shift();
					...
					rest : function(a, b) {
						...
						var e = FB.ApiServer._readOnlyCalls[c] ? 'api_read'
								: 'api';
						FB.ApiServer.oauthRequest(e, 'restserver.php', 'get',
								a, b);
					},
					oauthRequest : function(a, b, c, d, e) {
						if (!d.access_token && FB.getAccessToken())
							d.access_token = FB.getAccessToken();
						d.sdk = 'joey';
						d.pretty = 0;
						var f = e;
						e = function(i) {
							if (FB.Auth
									&& i
									&& FB.getAccessToken() == d.access_token
									&& (i.error_code === '190' || (i.error && (i.error === 'invalid_token' || i.error.type === 'OAuthException'))))
						...
						try {
							...
							try {
								if (!FB.initSitevars.corsKillSwitch
										&& FB.ApiServer.corsPost(a, b, c,
												FB.JSON.flatten(d), e))
									return;
							} catch (h) {
							}
							if (FB.Flash.hasMinVersion()) {
								FB.ApiServer.flash(a, b, c, FB.JSON.flatten(d),
										e);
							} else
								throw new Error(
										'Your browser does not support long connect '
					...
					corsPost : function(a, b, c, d, e) {
						...
						if (a == 'graph')
							d.method = c;
						var g = FB.QS.encode(d), h = 'application/x-www-form-urlencoded', i = FB.ApiServer
								._createCORSRequest('POST', f, h);
						if (i) {
							i.onload = function() {
					...
					flash : function(a, b, c, d, e) {
						if (!window.FB_OnXdHttpResult)
							window.FB_OnXdHttpResult = function(f, g) {
								FB.ApiServer._callbacks[f]
						...
						FB.Flash.onReady(function() {
							...
							var h = document.XdComm.sendXdHttpRequest(c
									.toUpperCase(), f, g, null);
				...
				'XD',
				{
					...
					init : function(a) {
						...
						if (window.addEventListener && !window.attachEvent
							...
							if (document.getElementById('fb-root')) {
								...
								if (FB.log)
									FB
											.log('missing fb-root, defaulting to fragment-based xdcomm');
						...
						if (FB.XD._transport != 'postmessage' && d
								&& window.postMessage) {
					...
					resolveRelation : function(a) {
						...
						for ( var f = 0, g = d.length; f < g; f++) {
							...
							if (b === 'opener' || b === 'parent' || b === 'top') {
								e = e[b];
							} else if (c = /^frames\[['"]?([a-zA-Z0-9-_]+)['"]?\]$/
									.exec(b)) {
								e = e.frames[c[1]];
							} else
								throw new SyntaxError(
										'Malformed id to resolve: ' + a
												+ ', pt: ' + b);
						}
						return e;
					},
					handler : function(a, b, c, d, e) {
						...
						if (FB.initSitevars.forceSecureXdProxy)
							e = true;
						var f = FB.getDomain((e ? 'https_' : '') + 'cdn')
								+ FB.XD._xdProxyUrl + '#';
						d = d || FB.guid();
						b = b || 'opener';
					...
					recv : function(a) {
						...
						if (!FB.XD._forever[a.cb])
							delete FB.XD._callbacks[a.cb];
						c && c(a);
					},
					PostMessage : {
						...
						init : function() {
							if (!FB.XD.PostMessage._isInitialized) {
								...
								window.addEventListener ? window
										.addEventListener('message', a, false)
									...
									.onReady(function() {
										document.XdComm
												.postMessage_init(
														...
														FB.XD._openerOrigin ? FB.XD._openerOrigin
																: FB.XD._origin);
									});
						},
					...
					Fragment : {
						...
						checkAndDispatch : function() {
							var a = window.location.toString(), b = a.substr(a
									.indexOf('#') + 1), c = a
							...
							if (c > 0) {
								...
								FB.XD.resolveRelation(FB.QS.decode(b).relation).FB.XD
										.recv(b);
							}
						}
					}
...
FB
		.provide(
				'UA',
				{
					...
					nativeApp : function() {
						return FB.UA.mobile()
								&& navigator.userAgent.match(/FBAN\/\w+;/i);
					},
					android : function() {
						FB.UA._populate();
					...
					_populate : function() {
						...
						var a = /(?:MSIE.(\d+\.\d+))|(?:(?:Firefox|GranParadiso|Iceweasel).(\d+\.\d+))|(?:Opera(?:.+Version.|.)(\d+\.\d+))|(?:AppleWebKit.(\d+(?:\.\d+)?))/
								.exec(navigator.userAgent), b = /(Mac OS X)|(Windows)|(Linux)/
								.exec(navigator.userAgent), c = /\b(iPhone|iP[ao]d)/
								.exec(navigator.userAgent);
						FB.UA._iPad = /\b(iPad)/.exec(navigator.userAgent);
						FB.UA._android = navigator.userAgent.match(/Android/i);
						FB.UA._mobile = c || FB.UA._android
						...
						if (a) {
							...
							if (FB.UA._safari) {
								a = /(?:Chrome\/(\d+\.\d+))/
										.exec(navigator.userAgent);
								FB.UA._chrome = a && a[1] ? parseFloat(a[1])
										: NaN;
							} else
								FB.UA._chrome = NaN;
						} else
					...
					inform : function(a, b, c, d, e) {
						if (FB.Canvas.isTabIframe()
							...
							if (window.postMessage) {
								FB.XD.resolveRelation(c || 'parent')
										.postMessage(f, '*');
								return;
							} else
								try {
						...
						d |= (window != window.parent && document.referrer
								.indexOf('https:') === 0);
						var h = (FB.getDomain((d ? 'https_' : '') + 'staticfb',
								true)
								+ FB.Arbiter._canvasProxyUrl + '#' + FB.QS
								.encode({
									method : a,
									params : FB.JSON.stringify(b || {}),
									behavior : e
											|| FB.Arbiter.BEHAVIOR_PERSISTENT,
									relation : c
		...
		.provide(
				'Canvas',
				{
					...
					getPageInfo : function(a) {
						var b = 'top.frames[' + window.name + ']', c = FB.XD
								.handler(function(e) {
									for ( var f in FB.Canvas._pageInfo)
										if (e[f])
											FB.Canvas._pageInfo[f] = e[f] | 0;
					...
					_hideFlashCallback : function(a) {
						...
						for ( var c = 0; c < b.length; c++) {
							var d = b[c];
							if (d.type.toLowerCase() != "application/x-shockwave-flash"
									&& d.classid.toUpperCase() != FB.Canvas._flashClassID)
								continue;
							var e = false;
							for ( var f = 0; f < d.childNodes.length; f++)
								if (d.childNodes[f].nodeName.toLowerCase() == "param"
										&& d.childNodes[f].name.toLowerCase() == "wmode")
									if (d.childNodes[f].value.toLowerCase() == "opaque"
											|| d.childNodes[f].value
													.toLowerCase() == "transparent")
										e = true;
							if (!e) {
								var g = Math.random();
								if (g <= 1 / 1000)
									FB.api(FB._apiKey + '/occludespopups',
											'post', {});
								if (FB.Canvas._devHideFlashCallback) {
									var h = 200, i = {
										...
										if (k.state == 'opened') {
											...
											FB.Canvas.showFlashElement(k.elem);
									}, this, i);
									setTimeout(j, h);
					...
					init : function() {
						...
						var b = FB.XD.handler(FB.Canvas._hideFlashCallback,
								'top.frames[' + window.name + ']', true);
					...
					setSize : function(a) {
						...
						if (FB.Canvas._lastSize[a.frame]) {
							var d = FB.Canvas._lastSize[a.frame].height, e = a.height
									- d;
							if (FB.Canvas._lastSize[a.frame].width == a.width
									&& (e <= c && e >= -b))
								return false;
						}
					...
					stopTimer : function(a) {
						FB.Canvas
								._passAppTtiMessage(a, 'StopIframeAppTtiTimer');
					},
					setUrlHandler : function(a) {
						var b = FB.XD.handler(a, 'top.frames[' + window.name
								+ ']', true);
					...
					startTimer : function() {
						FB.Canvas._passAppTtiMessage(null,
								'StartIframeAppTtiTimer');
					},
					_passAppTtiMessage : function(a, b) {
						var c = null;
						if (a)
							c = FB.XD.handler(a, 'top.frames[' + window.name
									+ ']', false);
						FB.Arbiter.inform(b, {
							frame : window.name || 'iframe_canvas',
					...
					_computeContentSize : function() {
						var a = document.body, b = document.documentElement, c = 0, d = Math
								.max(a.offsetTop, 0), e = Math.max(b.offsetTop,
								0), f = a.scrollHeight + d, g = a.offsetHeight
								+ d, h = b.scrollHeight + e, i = b.offsetHeight
								+ e;
						bottom = Math.max(f, g, h, i);
						if (a.offsetWidth < a.scrollWidth) {
							c = a.scrollWidth + a.offsetLeft;
						} else
							FB.Array.forEach(a.childNodes, function(j) {
				...
				'Dom',
				{
					...
					getByClass : function(a, b, c) {
						...
						if (b.querySelectorAll)
							return FB.Array.toArray(b.querySelectorAll(c + '.'
									+ a));
						var d = b.getElementsByTagName(c), e = [];
						for ( var f = 0, g = d.length; f < g; f++)
							if (this.containsCss(d[f], a))
								e[e.length] = d[f];
						return e;
					},
					getStyle : function(a, b) {
						...
						if (a.currentStyle) {
							FB.Array.forEach(b.match(/\-([a-z])/g),
									function(e) {
										b = b.replace(e, e.substr(1, 1)
												.toUpperCase());
									});
							c = a.currentStyle[b];
						} else {
...
FB.provide('Intl', (function() {
	...
	function c(e, f) {
		if (f !== undefined)
			if (typeof f != 'object') {
				FB.log('The second arg to FB.Intl.tx() must be an Object for '
						+ 'FB.Intl.tx(' + e + ', ...)');
			} else {
				var g;
				for ( var h in f)
...
FB.provide('', {
	bind : function() {
		var a = Array.prototype.slice.call(arguments), b = a.shift(), c = a
				.shift();
		return function() {
				...
				'Dialog',
				{
					...
					_showIPadOverlay : function() {
						...
						if (!FB.Dialog._overlayEl) {
							FB.Dialog._overlayEl = document
									.createElement('div');
							FB.Dialog._overlayEl.setAttribute('id',
					...
					showLoader : function(a, b) {
						...
						if (!FB.Dialog._loaderEl)
							FB.Dialog._loaderEl = FB.Dialog._findRoot(FB.UA
									.mobile() ? FB.Dialog._createMobileLoader()
									: FB.Dialog._createWWWLoader(b));
						if (!a)
							a = function() {
							};
					...
					_centerActive : function(a) {
						...
						if (!b)
							return;
						var c = FB.Dom.getViewportInfo(), d = parseInt(
								b.offsetWidth, 10), e = parseInt(
								b.offsetHeight, 10), f = c.scrollLeft
								+ (c.width - d) / 2, g = (c.height - e) / 2.5;
						if (f < g)
							g = f;
						var h = c.height - e - g, i = (c.height - e) / 2;
						if (a)
							i = a.scrollTop - a.offsetTop
									+ (a.clientHeight - e) / 2;
						if (i < g) {
							i = g;
						} else if (i > h)
					...
					_setDialogSizes : function() {
						if (!FB.UA.mobile() || FB.UA.iPad())
							return;
						for ( var a in FB.Dialog._dialogs)
							if (document.getElementById(a)) {
								var b = document.getElementById(a);
								b.style.width = FB.UIServer.getDefaultSize().width
										+ 'px';
								b.style.height = FB.UIServer.getDefaultSize().height
										+ 'px';
							}
					},
					_handleOrientationChange : function(a) {
						if (FB.UA.android()
							...
							window.setTimeout(
									FB.Dialog._handleOrientationChange, 50);
							return;
						}
						...
						if (FB.UA.iPad()) {
							...
							for ( var b in FB.Dialog._dialogs)
								if (document.getElementById(b))
									document.getElementById(b).style.width = FB.UIServer
					...
					_addOrientationHandler : function() {
						if (!FB.UA.mobile())
							return;
						var a = "onorientationchange" in window ? 'orientationchange'
								: 'resize';
						FB.Dialog._availScreenWidth = screen.availWidth;
						FB.Event.listen(window, a,
								FB.Dialog._handleOrientationChange);
					},
					create : function(a) {
						a = a || {};
						var b = document.createElement('div'), c = document
								.createElement('div'), d = 'fb_dialog';
						...
						if (FB.UA.ie()) {
							...
							FB.Array.forEach(
									[ 'vert_left', 'vert_right', 'horiz_top',
											...
											'bottom_right' ], function(h) {
										var i = document.createElement('span');
										i.className = 'fb_dialog_' + h;
										b.appendChild(i);
									});
						} else
							d += (FB.UA.mobile()) ? ' fb_dialog_mobile'
									: ' fb_dialog_advanced';
						if (a.content)
							FB.Content.append(a.content, c);
						b.className = d;
					...
					show : function(a) {
						...
						if (b) {
							...
							if ('fbCallID' in a)
								FB.Dialog.get(a.fbCallID).fire('iframe_show');
							if (!FB.Event._popStateListenerOn) {
								FB.Event.listen(window, 'popstate',
										FB.Dialog._hideOnPopState);
					...
					hide : function(a) {
						...
						if (b == FB.Dialog._active) {
							...
							FB.Dialog._hideIPadOverlay();
							if ('fbCallID' in a)
								FB.Dialog.get(a.fbCallID).fire('iframe_hide');
							if (FB.Event._popStateListenerOn) {
								FB.Event.unlisten(window, 'popstate',
										FB.Dialog._hideOnPopState);
					...
					remove : function(a) {
						...
						if (a) {
							...
							} else if (FB.Dialog._active === null
									&& FB.Dialog._stack.length > 0)
								FB.Dialog.show(FB.Dialog._stack.pop());
							window.setTimeout(function() {
		...
		.provide(
				'UIServer',
				{
					...
					prepareCall : function(a, b) {
						var c = a.method.toLowerCase(), d = FB.copy({},
								FB.UIServer.Methods[c]), e = FB.guid(), f = (d.noHttps !== true)
								&& (FB._https || (c !== 'auth.status' && c != 'login.status'));
						...
						FB.copy(a, {
							...
							locale : FB._locale,
							sdk : 'joey',
							access_token : f && FB.getAccessToken()
									|| undefined
						});
						...
						if (!(g.id in FB.UIServer._defaultCb)
								&& !('next' in g.params)
								&& !('redirect_uri' in g.params))
							g.params.next = FB.UIServer._xdResult(g.cb, g.id,
									j, true);
						if (j === 'parent')
							g.params.channel_url = FB.UIServer
									._xdChannelHandler(e, 'parent.parent');
					...
					prepareParams : function(a) {
						...
						if (FB.TemplateUI
								&& FB.TemplateUI.supportsTemplate(b, a)) {
							...
							if (!FB.UA.nativeApp()
									&& FB.UIServer.urlTooLongForIE(a.url + '?'
											+ c)) {
								a.post = true;
							} else if (c)
								a.url += '?' + c;
					...
					getDisplayMode : function(a, b) {
						if (b.display === 'hidden' || b.display === 'none')
							return b.display;
						if (FB.Canvas.isTabIframe() && b.display !== 'popup')
							return 'async';
						if (FB.UA.mobile() || b.display === 'touch')
							return 'touch';
						if (!FB.getAccessToken() && b.display == 'dialog'
								&& !a.loggedOutIframe) {
							FB
						...
						return b.display
								|| (FB.getAccessToken() ? 'dialog' : 'popup');
					},
					getXdRelation : function(a) {
						var b = a.display;
						if (b === 'popup' || b === 'touch')
							return 'opener';
						if (b === 'dialog' || b === 'iframe' || b === 'hidden'
								|| b === 'none')
							return 'parent';
						if (b === 'async')
					...
					popup : function(a) {
						var b = typeof window.screenX != 'undefined' ? window.screenX
								: window.screenLeft, c = typeof window.screenY != 'undefined' ? window.screenY
								: window.screenTop, d = typeof window.outerWidth != 'undefined' ? window.outerWidth
								: document.documentElement.clientWidth, e = typeof window.outerHeight != 'undefined' ? window.outerHeight
								: (document.documentElement.clientHeight - 22), f = FB.UA
								.mobile() ? null : a.size.width, g = FB.UA
								.mobile() ? null : a.size.height, h = (b < 0) ? window.screen.width
								+ b
								: b, i = parseInt(h + ((d - f) / 2), 10), j = parseInt(
								c + ((e - g) / 2.5), 10), k = [];
						if (f !== null)
							k.push('width=' + f);
						if (g !== null)
						...
						k.push('top=' + j);
						k.push('scrollbars=1');
						if (a.name == 'permissions.request'
								|| a.name == 'permissions.oauth')
							k.push('location=1,toolbar=0');
						k = k.join(',');
						if (a.post) {
							FB.UIServer.setLoadedNode(a, window.open(
									'about:blank', a.id, k), 'popup');
					...
					iframe : function(a) {
						...
						if (!a.hideLoader)
							FB.Dialog.showLoader(b, a.size.width);
					...
					touch : function(a) {
						if (a.params && a.params.in_iframe) {
							...
							FB.Native.onready(function() {
								FB.UIServer.setLoadedNode(a, FB.Native
										.open(a.url + '#cb=' + a.frameName));
							});
							FB.UIServer._popupMonitor();
						} else if (!a.ui_created)
							FB.UIServer.popup(a);
					},
					async : function(a) {
						a.frame = window.name;
					...
					_insertIframe : function(a) {
						...
						var b = function(c) {
							if (a.id in FB.UIServer._loadedNodes)
								FB.UIServer.setLoadedNode(a, c, 'iframe');
						};
						if (a.post) {
							FB.Content.insertIframe({
								url : 'about:blank',
					...
					_handleResizeMessage : function(a, b) {
						...
						FB.Arbiter.inform('resize.ack', b || {},
								'parent.frames[' + c.name + ']', true);
					...
					_popupMonitor : function() {
						var a;
						for ( var b in FB.UIServer._loadedNodes)
							if (FB.UIServer._loadedNodes.hasOwnProperty(b)
		...
		.provide(
				'Auth',
				{
					...
					fetchLoginStatus : function(a) {
						if (FB.UA.mobile() && window.postMessage
								&& window.localStorage) {
					...
					staticAuthCheck : function(a) {
						...
						FB.Content
								.insertIframe({
									...
									onload : function(c) {
										var d = frames[c.name], e = FB.guid(), f = false, g = function(
												h) {
											if (!f) {
												f = true;
												FB.Auth
														._staticAuthHandler(a,
																h);
											}
										};
					...
					setAuthResponse : function(a, b) {
						...
						if (a)
							if (a.userID) {
								...
								var d = FB.Auth
										.parseSignedRequest(a.signedRequest);
								if (d && d.user_id)
									c = d.user_id;
							}
						var e = !FB._userID && a, f = FB._userID && !a, g = a
								&& FB._userID != c, h = e || f || g, i = b != FB._userStatus, j = {
							authResponse : a,
							status : b
						};
						FB._authResponse = a;
						...
						FB._userStatus = b;
						if (f || g)
							FB.Event.fire('auth.logout', j);
						if (e || g)
							FB.Event.fire('auth.login', j);
						if (h)
							FB.Event.fire('auth.authResponseChange', j);
						if (i)
							FB.Event.fire('auth.statusChange', j);
						if (FB.Auth._refreshTimer) {
							window.clearTimeout(FB.Auth._refreshTimer);
							delete FB.Auth._refreshTimer;
						}
						if (FB.Auth._loadState && a)
							FB.Auth._refreshTimer = window.setTimeout(
									function() {
										FB.getLoginStatus(null, true);
									}, 1200000);
						return j;
					...
					xdHandler : function(a, b, c, d, e) {
						return FB.UIServer._xdNextHandler(FB.Auth
								.xdResponseWrapper(a, d, e), b, c, true);
					},
					xdResponseWrapper : function(a, b, c) {
						return function(d) {
							if (d.access_token) {
								var e = FB.Auth
										.parseSignedRequest(d.signed_request);
								b = {
								...
								if (FB.Cookie.getEnabled()) {
									var f = b.expiresIn === 0 ? 0
											: (new Date()).getTime()
													+ b.expiresIn * 1000, g = FB.Cookie._domain;
									if (!g && d.base_domain)
										g = '.' + d.base_domain;
									FB.Cookie.setSignedRequestCookie(
											d.signed_request, f, g);
								}
					...
					parseSignedRequest : function(a) {
						if (!a)
							return null;
						var b = a.split('.', 2), c = b[1], d = FB.Auth
								.base64URLDecode(c);
					...
					base64URLDecode : function(a) {
						...
						for ( var m = 0; m < a.length; m += 4) {
							...
							if (k != 64)
								e = e + String.fromCharCode(g);
							if (l != 64)
								e = e + String.fromCharCode(h);
							f = g = h = "";
							i = j = k = l = "";
						}
						return unescape(e);
...
FB.provide('UIServer.Methods', {
	'permissions.oauth' : {
		...
		transform : function(a) {
			...
			FB.copy(a.params, {
				...
				redirect_uri : FB.URI.resolve(FB.Auth.xdHandler(b, c, 'opener',
	...
	'auth.logout' : {
		...
		transform : function(a) {
			if (!FB._apiKey) {
				...
				a.params.next = FB.Auth.xdHandler(a.cb, a.id, 'parent',
						FB._authResponse);
				return a;
			}
		}
	...
	'login.status' : {
		...
		transform : function(a) {
			...
			FB.copy(a.params, {
				client_id : FB._apiKey,
				redirect_uri : FB.Auth.xdHandler(b, c, 'parent',
						FB._authResponse),
				origin : FB.Auth._getContextType(),
...
FB.provide('Cookie',
		{
			...
			loadMeta : function() {
				var a = document.cookie.match('\\bfbm_' + FB._apiKey
						+ '=([^;]*)\\b'), b;
				if (a) {
					b = FB.QS.decode(a[1]);
			...
			loadSignedRequest : function() {
				var a = document.cookie.match('\\bfbsr_' + FB._apiKey
						+ '=([^;]*)\\b');
				if (!a)
					return null;
				return a[1];
			...
			setSignedRequestCookie : function(a, b, c) {
				if (!a)
					throw new Error(
							'Value passed to FB.Cookie.setSignedRequestCookie '
									+ 'was empty.');
				if (!FB.Cookie.getEnabled())
					return;
				if (c) {
			...
			setRaw : function(a, b, c, d) {
				...
				document.cookie = a + FB._apiKey + '=' + b
						+ (b && c === 0 ? '' : '; expires=' + e) + '; path=/'
						+ (d ? '; domain=' + d : '');
			}
		});
FB.provide('Frictionless', {
	...
	_processRequestResponse : function(a, b) {
		return function(c) {
			...
			if (FB.Frictionless._useFrictionless && d)
				FB.Frictionless._updateRecipients();
			if (c) {
				if (!b && c.frictionless) {
					FB.Dialog._hideLoader();
	...
	isAllowed : function(a) {
		if (!a)
			return false;
		if (typeof a === 'number')
			return FB.Frictionless._allowedRecipients[a];
		if (typeof a === 'string')
			a = a.split(',');
		...
		.provide(
				'Canvas.Prefetcher',
				{
					...
					setCollectionMode : function(a) {
						if (!FB._inCanvas || !FB._apiKey)
							return false;
						if (a != FB.Canvas.Prefetcher.COLLECT_AUTOMATIC
								&& a != FB.Canvas.Prefetcher.COLLECT_MANUAL)
							return false;
						FB.Canvas.Prefetcher._collectionMode = a;
					},
					_maybeSample : function() {
						if (!FB._inCanvas || !FB._apiKey
								|| !FB.Canvas.Prefetcher._sampleRate)
							return;
						var a = Math.random();
					...
					_sample : function() {
						...
						if (FB.Canvas.Prefetcher._collectionMode == FB.Canvas.Prefetcher.COLLECT_AUTOMATIC)
							FB.Array.forEach(a, function(c, d) {
								FB.Array.forEach(window.document
										.getElementsByTagName(d), function(e) {
									if (e[c])
										FB.Canvas.Prefetcher._links.push(e[c]);
								});
							});
...
FB.provide('', {
	...
	init : function(a) {
		...
		if (FB.UA.mobile() && FB.TemplateUI && FB.TemplateData
				&& FB.TemplateData._enabled && a.useCachedDialogs !== false) {
			FB.TemplateUI.init();
		...
		if (FB._apiKey) {
			...
			if (a.authResponse) {
				...
				var b = FB.Cookie.loadSignedRequest(), c = FB.Auth
						.parseSignedRequest(b);
...
FB.provide('UIServer.MobileIframableMethod', {
	transform : function(a) {
		if (a.params.display === 'touch' && a.params.access_token
				&& window.postMessage) {
	...
	getXdRelation : function(a) {
		var b = a.display;
		if (b === 'touch' && window.postMessage && a.in_iframe)
			return 'parent';
...
FB.provide('UIServer.Methods', {
	...
	'auth.logintofacebook' : {
		...
		transform : function(a) {
			...
			var b = FB.UIServer.getXdRelation(a.params), c = FB.UIServer
					._xdResult(a.cb, a.id, b, true);
			a.params.next = FB.getDomain(FB._https ? 'https_www' : 'www')
	...
	apprequests : {
		transform : function(a) {
			...
			a.params.frictionless = FB.Frictionless
		...
		.provide(
				'XFBML',
				{
					...
					getElements : function(a, b, c) {
						var d = FB.Array, e = FB.XFBML._getDomElements(a, b, c), f = FB.Dom
								.getByClass(b + '-' + c, a, 'div');
						e = d.toArray(e);
						f = d.toArray(f);
						f = d
								.filter(
										f,
										function(g) {
											return !g.hasChildNodes()
													|| (g.childNodes.length === 1 && g.childNodes[0].nodeType === 3);
										});
						return d.merge(e, f);
					},
					parse : function(a, b) {
						...
						FB.Array.forEach(FB.XFBML._tagInfos, function(f) {
							...
							if (e[f.localName] !== undefined) {
								...
								g = FB.XFBML.getElements(a, f.xmlns,
										f.localName);
						...
						window.setTimeout(function() {
							if (c > 0)
								FB.log(c + ' XFBML tags failed to render in '
					...
					getBoolAttr : function(a, b) {
						...
						return (b && FB.Array.indexOf([ 'true', '1', 'yes',
								'on' ], b.toLowerCase()) > -1);
					},
						...
						if (element) {
							...
							var processor = function() {
								...
								if ((tagInfo.className === 'FB.XFBML.LoginButton')
										|| addToTimeline) {
									renderInIframe = FB.XFBML.getBoolAttr(dom,
											'render-in-iframe');
									mode = FB.XFBML.getAttr(dom, 'mode');
									showFaces = (addToTimeline && mode != 'button')
											|| FB.XFBML.getBoolAttr(dom,
													'show-faces');
									showLoginFace = FB.XFBML.getBoolAttr(dom,
											'show-login-face');
								...
								if (isLogin) {
									...
									var extraParams = {
										...
										mode : mode
									}, scope = FB.XFBML.getAttr(dom, 'scope')
					...
					_getDomElements : function(a, b, c) {
						var d = b + ':' + c;
						if (FB.UA.firefox()) {
							return a.getElementsByTagNameNS(
									document.body.namespaceURI, d);
						} else if (FB.UA.ie() < 9) {
							try {
...
FB
		.subclass(
				...
				{
					parse : function(a) {
						var b = FB.String.format.apply(null, a), c = (/^select (.*?) from (\w+)\s+where (.*)$/i)
								.exec(b);
						this.fields = this._toFields(c[1]);
						this.table = c[2];
						this.where = this._parseWhere(c[3]);
						for ( var d = 1; d < a.length; d++)
							if (FB.Type.isType(a[d], FB.Data.Query))
								a[d].hasDependency = true;
						return this;
					},
					toFql : function() {
	...
	_process : function() {
		...
		FB.api('/fql', 'GET', e, function(f) {
			if (f.error) {
				...
				FB.Array.forEach(f.data, function(g) {
					a[g.name].set(g.fql_result_set);
				});
		});
	},
	_mergeIndexQuery : function(a, b) {
		var c = a.where.key, d = a.where.value, e = 'index_' + a.table + '_'
				+ c, f = b[e];
		if (!f) {
		...
		.provide(
				'Native',
				{
					...
					onready : function(a) {
						if (!FB.UA.nativeApp()) {
							FB
									.log('FB.Native.onready only works when the page is rendered '
						...
						if (this.nativeReady) {
							...
							var b = function(c) {
								window.removeEventListener(
										FB.Native.NATIVE_READY_EVENT, b);
								FB.Native.onready(a);
							};
							window.addEventListener(
									FB.Native.NATIVE_READY_EVENT, b, false);
						}
					}
				});
FB.provide('UIServer.Methods', {
...
FB.provide('Helper',
		{
			...
			getProfileLink : function(a, b, c) {
				c = c
						|| (a ? FB.getDomain('www') + 'profile.php?id=' + a.uid
								: null);
				if (c)
					b = '<a class="fb_link" href="' + c + '">' + b + '</a>';
				return b;
			},
...
FB.provide('TemplateData', {
	...
	_enabled : true,
	enabled : function() {
		return FB.TemplateData._enabled && FB.TemplateData._initialized
	...
	_isStale : function(a) {
		if (!a || !a.version || a.version != FB.TemplateData._version
				|| a.currentUserID != FB.getUserID())
			return true;
	...
	getResponse : function() {
		...
		try {
			a._response = a._response
					|| (a.supportsLocalStorage() && FB.JSON
							.parse(localStorage.FB_templateDataResponse
									|| "null"));
		} catch (b) {
			a._response = null;
		}
		if (a._isStale(a._response))
			a.saveResponse(null);
		return a._response;
	},
	saveResponse : function(a) {
		...
		if (FB.TemplateData.supportsLocalStorage())
			localStorage.FB_templateDataResponse = FB.JSON.stringify(a);
	},
	getData : function() {
...
FB.subclass('TemplateUI', 'Obj', function(a, b) {
	...
	if (window.devicePixelRatio)
		d.m_pixel_ratio = window.devicePixelRatio;
	...
	FB.XD.handler(this.bind(function(g) {
		if (g.type == 'getParams')
			this.setProperty('getParamsCb', g.returnCb);
	...
	if (c) {
...
},
		{
			use : function(a) {
				...
				FB.UIServer.setLoadedNode(a, FB.UIServer
						.getLoadedNode(this.cachedCall.id));
				...
				d.frictionless = FB.TemplateUI.isFrictionlessAppRequest(
						this.method, d);
			...
			setParams : function(a) {
				this.monitor('getParamsCb', this.bind(function() {
					if (this.getParamsCb) {
						var b = frames[this.cachedCall.frameName]
...
FB.provide('TemplateUI', {
	...
	populateCache : function(a) {
		...
		for ( var c in b)
			if (!(c in FB.TemplateUI._cache))
				FB.TemplateUI._cache[c] = new FB.TemplateUI(c, a);
	},
	initCache : function() {
	...
	supportsTemplate : function(a, b) {
		return FB.TemplateData.enabled()
				&& FB.TemplateUI.paramsAllowTemplate(a, b.params)
				&& b.params.display === 'touch' && FB.UA.mobile();
	},
	...
	isFrictionlessAppRequest : function(a, b) {
		return a === 'apprequests' && FB.Frictionless
				&& FB.Frictionless._useFrictionless;
	},
	willWriteOnGet : function(a, b) {
		return FB.TemplateUI.isFrictionlessAppRequest(a, b) && b.to
				&& FB.Frictionless.isAllowed(b.to);
	}
});
FB.provide('URI', {
	resolve : function(a) {
...
FB.Class('XFBML.Element', function(a) {
	...
	_getBoolAttribute : function(a, b) {
		if (FB.XFBML.getAttr(this.dom, a) === null)
			return b;
		...
		.subclass(
				...
				null,
				{
					...
					getIframeName : function() {
						if (!this._iframeName && this._widgetPipeEnabled
								&& FB.XFBML.shouldUseWidgetPipe()) {
							this._iframeName = this
									.generateWidgetPipeIframeName();
					...
					arbiterInform : function(event, a, b) {
						if (this.loaded) {
							...
							this.subscribe('iframe.onload', FB.bind(
									this._arbiterInform, this, event, a, b));
					},
					_arbiterInform : function(event, a, b) {
						var c = 'parent.frames["' + this.getIframeNode().name
								+ '"]';
						FB.Arbiter.inform(event, a, c,
								window.location.protocol == 'https:', b);
					},
					...
					_getURL : function() {
						...
						return FB.getDomain(a) + 'plugins/' + b
				...
				'XFBML.IframeWidget',
				{
					...
					batchWidgetPipeRequests : function() {
						if (!FB.XFBML.IframeWidget.masterWidgetPipeIframe)
							return;
						var a = FB.XFBML.IframeWidget
								._groupWidgetPipeDescriptions(), b = {
							widget_pipe : FB.JSON.stringify(a),
							href : window.location,
							site : location.hostname,
							channel : FB.XFBML.IframeWidget.masterWidgetPipeIframe
									.getChannelUrl(),
							api_key : FB._apiKey,
							locale : FB._locale,
							sdk : 'joey'
						}, c = FB.guid(), d = FB.XFBML.IframeWidget.masterWidgetPipeIframe.dom, e = d
					...
					_groupWidgetPipeDescriptions : function() {
						...
						for ( var b in FB.XFBML.IframeWidget.allWidgetPipeIframes) {
							var c = FB.XFBML.IframeWidget.allWidgetPipeIframes[b], d = c
									.getUrlBits(), e = {
								widget : d.name
							};
							FB.copy(e, d.params);
		...
		.subclass(
				...
				null,
				{
					...
					process : function() {
						...
						var a = this._getAttributeFromList('size', 'medium',
								this._allowedSizes), b = '', c = '';
						if (a == 'icon') {
							b = 'fb_button_simple';
						} else {
						...
						if (c !== '') {
							this.dom.innerHTML = ('<a class="' + b + '">'
									+ '<span class="fb_button_text">' + c
									+ '</span>' + '</a>');
...
FB.subclass('XFBML.Comments', 'XFBML.IframeWidget', null, {
	...
	setupAndValidate : function() {
		...
		if (FB.initSitevars.enableMobileComments && FB.UA.mobile()
				&& a.mobile !== false)
			a.mobile = true;
		if (!a.href) {
			...
			if (a.migrated)
				a.href = 'http://www.facebook.com/plugins/comments_v1.php?'
						+ 'app_id=' + FB._apiKey + '&xid='
						+ encodeURIComponent(a.xid) + '&url='
						+ encodeURIComponent(a.url);
		} else {
			var c = this.getAttribute('fb_comment_id');
			if (!c) {
				c = FB.QS.decode(document.URL.substring(document.URL
						.indexOf('?') + 1)).fb_comment_id;
				if (c && c.indexOf('#') > 0)
	...
	oneTimeSetup : function() {
		...
		this.subscribe('xd.commentCreated', FB.bind(
				this._handleCommentCreatedMsg, this));
		this.subscribe('xd.commentRemoved', FB.bind(
				this._handleCommentRemovedMsg, this));
	},
	getSize : function() {
...
FB.subclass('XFBML.CommentsCount', 'XFBML.Element', null, {
	process : function() {
		...
		this._count = FB.Data._selectByIndex([ 'commentsbox_count' ],
				'link_stat', 'url', this._href);
		...
		this._count.wait(FB.bind(function() {
			...
			this.dom.innerHTML = FB.String.format(
					'<span class="fb_comments_count">{0}</span>', a);
			if (a > 0)
...
FB.provide('Anim', {
	ate : function(a, b, c, d) {
		...
		var e = 40, f = {}, g = {}, h = null, i = a.style, j = setInterval(FB
				.bind(function() {
					if (!h)
						h = new Date().getTime();
					var k = 1;
					if (c != 0)
						k = Math.min((new Date().getTime() - h) / c, 1);
					FB.Array.forEach(b, FB.bind(function(l, m) {
						...
						if (!g[m])
							g[m] = this._parseCSS(l.toString());
						var o = '';
						FB.Array.forEach(f[m], function(p, q) {
							if (isNaN(g[m][q].numPart)
									&& g[m][q].textPart == '?') {
...
FB.provide('Insights',
		{
			impression : function(a, b) {
				var c = FB.guid(), d = "//ah8.facebook.com/impression.php/" + c
						+ "/", e = new Image(1, 1), f = [];
				...
				null,
				{
					...
					_showBar : function() {
						var a = FB.Data._selectByIndex([ 'first_name',
								...
								'uid', this._uid), b = FB.Data._selectByIndex(
								[ 'display_name' ], 'application', 'api_key',
								FB._apiKey);
					...
					_renderConnectBar : function(a) {
						...
						b.innerHTML = FB.String
								.format(
										...
										FB.Intl.tx._("Close"),
										a[this._picFieldName]
												|| FB.getDomain('cdn')
												+ FB.XFBML.ConnectBar.imgs.missingProfileUrl,
										FB.String.escapeHTML(a.first_name),
										FB.Intl.tx
												._(
														"Hi {firstName}. \u003Cstrong>{siteName}\u003C\/strong> is using Facebook to personalize your experience.",
														{
															...
															siteName : FB.String
																	.escapeHTML(a.site_name)
														}), FB.Intl.tx
												._("Learn More"),
										a.profile_url, FB.getDomain('www')
												+ 'sitetour/connect.php');
						var d = this;
						...
						if (this._page.parentNode) {
							...
							e = parseInt(FB.Dom.getStyle(this._page,
									'marginTop'), 10);
						e = isNaN(e) ? 0 : e;
						...
						if (FB.UA.ie()) {
							...
							f.backgroundPosition = '? ' + this._initialHeight
									+ 'px';
		...
		.subclass(
				...
				null,
				{
					...
					_getWidgetHeight : function() {
						var a = this._getLayout(), b = this._shouldShowFaces() ? 'show'
								: 'hide', c = this._getBoolAttribute('send'), d = 65 + (c ? 25
								: 0), e = {
							standard : {
								show : 80,
								hide : 35
					...
					_getWidgetWidth : function() {
						var a = this._getLayout(), b = this
								...
								.getAttribute('action') === 'recommend'), e = (d ? 265
								: 225)
								+ (b ? 60 : 0), f = (d ? 130 : 90)
								+ (b ? 60 : 0), g = this.getAttribute('action') === 'recommend' ? 100
								: 55, h = this.getAttribute('action') === 'recommend' ? 90
								: 50, i = {
							standard : {
								show : 450,
								hide : 450
...
FB
		.subclass(
				'XFBML.SendButtonFormWidget',
				'XFBML.EdgeCommentWidget',
				function(a) {
					this._base(a);
					...
					FB.Dom
							.addCss(
									this.dom,
									(typeof a.siderender != 'undefined' && a.siderender) ? 'siderender'
											: '');
				...
				},
				{
					...
					oneTimeSetup : function() {
						this.subscribe('xd.messageSent', FB.bind(
								this._onMessageSent, this));
					},
		...
		.subclass(
				...
				null,
				{
						...
						if (!this._attr.registration_url) {
							...
							case 'unknown':
								FB.ui({
									...
									FB.bind(FB.getLoginStatus(this._saveStatus(
											this._authCallback, true), true),
											this);
								}, this));
								break;
							case 'notConnected':
							...
							case 'connected':
								if (!FB.getAccessToken()
									...
									FB
											.logout(FB.bind(this._authCallback,
													this));
								break;
							default:
								FB.log('Unknown status: ' + this._attr.status);
					...
					_authCallback : function(a) {
						FB.Helper
								.invokeHandler(this._attr.onlogin, this, [ a ]);
					},
		...
		.subclass(
				...
				null,
				{
					process : function() {
						...
						var g = this
								.bind(function(h) {
									var i = h ? h[0] : null, j = i ? i[b]
											: null;
									if (!j)
										j = FB.getDomain('cdn')
												+ FB.XFBML.ProfilePic._defPicMap[b];
									var k = ((c ? 'width:' + c + ';' : '') + (d ? 'height:'
											+ c + ';'
											: '')), l = FB.String
											.format(
		...
		.subclass(
				...
				null,
				{
					...
					getSize : function() {
						return {
							height : 25,
							width : (this._attr.action == 'recommend' ? 140
									: 96)
						};
					},
