					...
					insertIframe : function(e) {
						...
						if (document.attachEvent) {
							var b = ('<iframe'
									...
									+ '()"' + '></iframe>');
							e.root.innerHTML = '<iframe src="javascript:false"'
		...
		.provide(
				'Flash',
				{
					...
					embedSWF : function(d, e, b) {
						var a = !!document.attachEvent, c = ('<object '
					...
					hasMinVersion : function() {
						if (typeof FB.Flash._hasMinVersion === 'undefined') {
							...
							if (i) {
								var f = i.replace(/\D+/g, ',').match(
										/^,?(.+),?$/)[1].split(',');
								for (a = 0, b = f.length; a < b; a++)
									h.push(parseInt(f[a], 10));
							}
							FB.Flash._hasMinVersion = false;
							majorVersion: for (a = 0,
									b = FB.Flash._minVersions.length; a < b; a++) {
								var g = FB.Flash._minVersions[a];
								if (g[0] != h[0])
									continue;
								for ( var c = 1, d = g.length, e = h.length; (c < d && c < e); c++)
									if (h[c] < g[c]) {
										FB.Flash._hasMinVersion = false;
...
FB
		.provide(
				'ApiServer',
				{
					...
					graph : function() {
						var a = Array.prototype.slice.call(arguments), f = a
								.shift(), d = a.shift(), c, e, b;
						while (d) {
							var g = typeof d;
							if (g === 'string' && !c) {
								c = d.toLowerCase();
					...
					rest : function(e, a) {
						...
						var b = FB.ApiServer._readOnlyCalls[c] ? 'api_read'
								: 'api';
						FB.ApiServer.oauthRequest(b, 'restserver.php', 'get',
								e, a);
					},
					oauthRequest : function(b, g, d, f, a) {
						if (!f.access_token && FB.getAccessToken())
							f.access_token = FB.getAccessToken();
						f.sdk = 'joey';
						f.pretty = 0;
						var e = a;
						a = function(h) {
							if (FB.Auth
									&& h
									&& FB._session
									&& FB._session.access_token == f.access_token
									&& (h.error_code === '190' || (h.error && (h.error === 'invalid_token' || h.error.type === 'OAuthException'))))
						...
						try {
							...
							try {
								if (!FB.initSitevars.corsKillSwitch
										&& FB.ApiServer.corsPost(b, g, d,
												FB.JSON.flatten(f), a))
									return;
							} catch (e2_ignore) {
							}
							if (FB.Flash.hasMinVersion()) {
								FB.ApiServer.flash(b, g, d, FB.JSON.flatten(f),
										a);
							} else
								throw new Error(
					...
					flash : function(b, e, c, d, a) {
						...
						FB.Flash.onReady(function() {
							...
							var g = document.XdComm.sendXdHttpRequest(c
									.toUpperCase(), h, f, null);
		...
		.provide(
				'XD',
				{
					...
					init : function(b) {
						...
						if (FB.XD._transport != 'postmessage' && a
								&& window.postMessage) {
					...
					handler : function(a, f, c, d, b) {
						...
						if (FB.initSitevars.forceSecureXdProxy)
							b = true;
						var g = FB.getDomain((b ? 'https_' : '') + 'cdn')
								+ FB.XD._xdProxyUrl + '#';
						d = d || FB.guid();
						f = f || 'opener';
...
FB
		.provide(
				'UA',
				{
					...
					mobile : function() {
						FB.UA._populate();
						return !FB._inCanvas && FB.UA._enableMobile
								&& this._mobile;
					},
					nativeApp : function() {
						return FB.UA.mobile()
					...
					_populate : function() {
						...
						FB.UA._mobile = b || FB.UA._android
...
FB.provide('Arbiter', {
	...
	inform : function(d, f, g, c, a) {
		if (FB.Canvas.isTabIframe() || (FB._inPlugin && window.postMessage)
		...
		var i = (FB.getDomain((c ? 'https_' : '') + 'staticfb')
				+ FB.Arbiter._canvasProxyUrl + '#' + FB.QS.encode({
			method : d,
				...
				'Canvas',
				{
					...
					_hideFlashCallback : function(g) {
						...
						for ( var e = 0; e < a.length; e++) {
							var c = a[e];
							if (c.type.toLowerCase() != "application/x-shockwave-flash"
									&& c.classid.toUpperCase() != FB.Canvas._flashClassID)
								continue;
							var d = false;
							for ( var f = 0; f < c.childNodes.length; f++)
								if (c.childNodes[f].nodeName.toLowerCase() == "param"
										&& c.childNodes[f].name.toLowerCase() == "wmode")
									if (c.childNodes[f].value.toLowerCase() == "opaque"
											|| c.childNodes[f].value
					...
					init : function() {
						...
						var a = FB.XD.handler(FB.Canvas._hideFlashCallback,
					...
					setSize : function(e) {
						...
						if (FB.Canvas._lastSize[e.frame]) {
							...
							var a = e.height - d;
							if (FB.Canvas._lastSize[e.frame].width == e.width
									&& (a <= b && a >= -c))
								return false;
						}
					...
					_passAppTtiMessage : function(a, c) {
						var b = null;
						if (a)
							b = FB.XD.handler(a, 'top.frames[' + window.name
									+ ']', false);
						FB.Arbiter.inform(c, {
							frame : window.name || 'iframe_canvas',
					...
					_computeContentSize : function() {
						var a = document.body, e = document.documentElement, i = 0, d = Math
								.max(a.offsetTop, 0), h = Math.max(e.offsetTop,
								0), c = a.scrollHeight + d, b = a.offsetHeight
								+ d, g = e.scrollHeight + h, f = e.offsetHeight
								+ h;
				...
				'Dom',
				{
					...
					getByClass : function(b, c) {
						...
						if (c.getElementsByClassName)
							return FB.Array
									.toArray(c.getElementsByClassName(b));
				...
				'Dialog',
				{
					...
					create : function(e) {
						...
						if (FB.UA.ie()) {
							...
							a += (FB.UA.mobile()) ? ' fb_dialog_mobile'
									: ' fb_dialog_advanced';
						if (e.content)
		...
		.provide(
				'UIServer',
				{
					...
					prepareCall : function(h, b) {
						var g = h.method.toLowerCase(), f = FB.copy({},
								FB.UIServer.Methods[g]), e = FB.guid(), c = (f.noHttps !== true)
								&& (FB._https || (g !== 'auth.status' && g != 'login.status'));
						...
						FB.copy(h, {
							...
							locale : FB._locale,
							sdk : 'joey',
							access_token : c && FB.getAccessToken()
									|| undefined
						});
					...
					prepareParams : function(a) {
						...
						if (FB.TemplateUI
								&& FB.TemplateUI.supportsTemplate(c, a)) {
							if (FB.reportTemplates)
								console.log("Using template for " + c + ".");
					...
					popup : function(b) {
						...
						var a = typeof window.screenX != 'undefined' ? window.screenX
								...
								: document.documentElement.clientWidth, f = typeof window.outerHeight != 'undefined' ? window.outerHeight
								: (document.documentElement.clientHeight - 22), k = FB.UA
								.mobile() ? null : b.size.width, d = FB.UA
								.mobile() ? null : b.size.height, h = (a < 0) ? window.screen.width
								+ a
								: a, e = parseInt(h + ((g - k) / 2), 10), j = parseInt(
								i + ((f - d) / 2.5), 10), c = [];
						if (k !== null)
							c.push('width=' + k);
						if (d !== null)
						...
						c.push('top=' + j);
						c.push('scrollbars=1');
						if (b.name == 'permissions.request'
								|| b.name == 'permissions.oauth')
							c.push('location=1,toolbar=0');
						c = c.join(',');
						if (b.post) {
							FB.UIServer.setLoadedNode(b, window.open(
									'about:blank', b.id, c));
							...
							FB.UIServer.setLoadedNode(b, window.open(b.url,
									b.id, c));
					...
					getDefaultSize : function() {
						if (FB.UA.mobile())
							if (FB.UA.iPad()) {
								...
								return {
									width : c,
									height : Math.max(a, (b ? screen.width
											: screen.height))
								};
							}
						return {
					...
					_insertIframe : function(b) {
						...
						var a = function(c) {
							if (b.id in FB.UIServer._loadedNodes)
								FB.UIServer.setLoadedNode(b, c);
						};
						if (b.post) {
							FB.Content.insertIframe({
								url : 'about:blank',
					...
					_handleResizeMessage : function(b, a) {
						...
						FB.Arbiter.inform('resize.ack', a || {},
...
FB.provide('', {
	...
	getSession : function() {
		if (FB._oauth)
			throw new Error('FB.getSession incompatible with OAuth2.');
		return FB._session;
	},
	getAuthResponse : function() {
		if (!FB._oauth)
			throw new Error('getAuthResponse called when OAuth2 support is '
					+ 'disabled.  Please fix.');
	...
	login : function(a, b) {
		if (FB._oauth) {
			if (b && b.perms)
				throw new Error('OAuth2 specification states that \'perms\' '
				...
				'Auth',
				{
					...
					setSession : function(e, g) {
						...
						var b = !FB._session && e, c = FB._session && !e, a = FB._session
								...
								|| c
								|| (FB._session && e && FB._session.access_token != e.access_token), h = g != FB._userStatus;
						var d = {
							session : e,
							status : g
						};
					...
					setAuthResponse : function(a, h) {
						...
						var d = !FB._userID && a, e = FB._userID && !a, c = a
								&& FB._userID != j, b = d || e || c, i = h != FB._userStatus;
						var g = {
							authResponse : a,
							status : h
						};
						...
						FB._userStatus = h;
						if (e || c)
							FB.Event.fire('auth.logout', g);
						if (d || c)
							FB.Event.fire('auth.login', g);
						if (b)
							FB.Event.fire('auth.authResponseChange', g);
						if (i)
							FB.Event.fire('auth.statusChange', g);
						if (FB.Auth._refreshTimer) {
							window.clearTimeout(FB.Auth._refreshTimer);
							delete FB.Auth._refreshTimer;
						}
					...
					xdNewHandler : function(b, c, d, a) {
						if (!FB._oauth)
							throw new Error(
									'xdNewHandler should not be invoked unless '
											+ 'OAuth2 is being used.');
						return FB.UIServer._xdNextHandler(FB.Auth
								.xdNewResponseWrapper(b, a), c, d, true);
					},
					xdNewResponseWrapper : function(b, a) {
						if (!FB._oauth)
							throw new Error(
									'xdNewResponseWrapper should not be invoked unless '
											+ 'OAuth2 is being used.');
						return function(d) {
							if (d.access_token) {
								var e = FB.Auth
				...
				'UIServer.Methods',
				{
					'permissions.request' : {
						...
						transform : function(a) {
							...
							a.cb = FB.Auth.xdResponseWrapper(a.cb,
									FB._userStatus, FB._session);
					...
					'permissions.oauth' : {
						...
						transform : function(a) {
							...
							FB.copy(a.params, {
								...
								redirect_uri : FB.Auth.xdNewHandler(b, c,
										'opener'),
					...
					'auth.logout' : {
						...
						transform : function(a) {
							if (!FB._apiKey) {
								...
								FB
										.log('FB.logout() called without an access token.');
							} else if (!FB._oauth && !FB._session) {
								FB.log('FB.logout() called without a session.');
							} else {
								if (FB._oauth) {
									a.params.next = FB.Auth.xdNewHandler(a.cb,
											a.id, 'parent');
								} else
									a.params.next = FB.Auth.xdHandler(a.cb,
											a.id, 'parent', false, 'unknown');
								return a;
							}
						}
					},
...
FB.provide('Cookie',
		{
			...
			setSignedRequestCookie : function(b, a) {
				if (!FB._oauth)
					throw new Error(
							'FB.Cookie.setSignedRequestCookie should only be '
									+ 'used with OAuth2.');
				if (!b)
					throw new Error(
			...
			setRaw : function(c, e, d, a) {
				...
				document.cookie = c + FB._apiKey + '=' + e
						+ (e && d === 0 ? '' : '; expires=' + b) + '; path=/'
						+ (a ? '; domain=.' + a : '');
			},
			set : function(a) {
				if (!a) {
				...
				FB.Cookie.setRaw('fbs_', '"' + FB.QS.encode(a) + '"',
						a.expires, a.base_domain);
...
FB.provide('Frictionless', {
	...
	isAllowed : function(c) {
		if (!c)
			return false;
		if (typeof c === 'number')
			return FB.Frictionless._allowedRecipients[c];
		if (typeof c === 'string')
			c = c.split(',');
	...
	init : function(a) {
		...
		if (FB._apiKey) {
			...
			if (FB._oauth) {
				...
				FB.Auth.setSession(a.session, a.session ? 'connected'
						: 'unknown');
			}
			if (a.status)
				FB.getLoginStatus();
...
FB.provide('UIServer.MobileIframableMethod', {
	...
	getXdRelation : function(b) {
		var a = b.display;
		if (a === 'touch' && window.postMessage && b.in_iframe)
			return 'parent';
		...
		.provide(
				'XFBML',
				{
					...
					getElements : function(b, f, d) {
						var a = FB.Array, e = FB.XFBML._getDomElements(b, f, d), c = FB.Dom
								.getByClass(f + '-' + d, b);
						e = a.toArray(e);
						c = a.toArray(c);
						c = a
					...
					parse : function(d, b) {
						...
						FB.Array.forEach(FB.XFBML._tagInfos, function(g) {
							...
							if (a[g.localName] !== undefined) {
								...
								h = FB.XFBML.getElements(d, g.xmlns,
										g.localName);
					...
					getBoolAttr : function(b, a) {
						...
						return (a && FB.Array.indexOf([ 'true', '1', 'yes',
								'on' ], a.toLowerCase()) > -1);
					},
					getAttr : function(b, a) {
						return (b.getAttribute(a)
								|| b.getAttribute(a.replace(/_/g, '-'))
								|| b.getAttribute(a.replace(/-/g, '_'))
								|| b.getAttribute(a.replace(/-/g, ''))
								|| b.getAttribute(a.replace(/_/g, ''))
								|| b.getAttribute('data-' + a)
								|| b.getAttribute('data-'
										+ a.replace(/_/g, '-'))
								|| b.getAttribute('data-'
										+ a.replace(/-/g, '_'))
							...
							var processor = function() {
								...
								if (isLogin) {
									...
									if (scope) {
										...
										var perms = FB.XFBML.getAttr(dom,
												'perms');
										if (perms)
...
FB.provide('TemplateData', {
	...
	enabled : function() {
		return FB.TemplateData._enabled && FB.TemplateData._initialized
				...
				&& FB.TemplateData.getData().apprequests.template_data_enabled;
	},
	...
	_isStale : function(b) {
		if (!b || !b.version || b.version != FB.TemplateData._version
				|| b.currentUserID != FB.getUserID())
			return true;
	...
	getResponse : function() {
		...
		try {
			b._response = b._response
					|| (b.supportsLocalStorage() && FB.JSON
							.parse(localStorage.FB_templateDataResponse
									|| "null"));
		} catch (a) {
			b._response = null;
		}
		if (b._isStale(b._response))
			b.saveResponse(null);
		return b._response;
	},
...
FB.subclass('TemplateUI', 'Obj', function(d, c) {
	...
	if (window.devicePixelRatio)
		e.m_pixel_ratio = window.devicePixelRatio;
	...
	if (b) {
...
},
		{
			use : function(a) {
				...
				FB.UIServer.setLoadedNode(a,
						FB.UIServer._loadedNodes[this.cachedCall.id]);
...
FB.provide('TemplateUI', {
	...
	populateCache : function(a) {
		...
		for ( var b in c)
			if (!(b in FB.TemplateUI._cache))
				FB.TemplateUI._cache[b] = new FB.TemplateUI(b, a);
	},
	initCache : function() {
	...
	supportsTemplate : function(b, a) {
		return FB.TemplateData.enabled()
				&& FB.TemplateUI.paramsAllowTemplate(b, a.params)
				&& a.params.display === 'touch' && FB.UA.mobile();
	},
	...
	isFrictionlessAppRequest : function(b, a) {
		return b === 'apprequests' && FB.Frictionless
				&& FB.Frictionless._useFrictionless;
	},
	willWriteOnGet : function(b, a) {
		return FB.TemplateUI.isFrictionlessAppRequest(b, a) && a.to
				&& FB.Frictionless.isAllowed(a.to);
	}
});
FB.provide('URI', {
	resolve : function(b) {
...
FB.Class('XFBML.Element', function(a) {
	...
	_getBoolAttribute : function(b, a) {
		if (FB.XFBML.getAttr(this.dom, b) === null)
			return a;
					...
					arbiterInform : function(event, b, a) {
						if (this.loaded) {
							...
							this.subscribe('iframe.onload', FB.bind(
									this._arbiterInform, this, event, b, a));
					},
					_arbiterInform : function(event, b, a) {
						var c = 'parent.frames["' + this.getIframeNode().name
								+ '"]';
						FB.Arbiter.inform(event, b, c,
								window.location.protocol == 'https:', a);
					},
...
FB.subclass('XFBML.Comments', 'XFBML.IframeWidget', null, {
	...
	setupAndValidate : function() {
		...
		if (!a.href) {
			...
			if (!b) {
				b = FB.QS.decode(document.URL.substring(document.URL
					...
					_renderConnectBar : function(d) {
						...
						b.innerHTML = FB.String
								.format(
										...
										FB.String.escapeHTML(d.first_name),
										FB.Intl
												._tx(
														"Hi {firstName}. \u003cstrong>{siteName}\u003c\/strong> is using Facebook to personalize your experience.",
														{
															firstName : FB.String
																	.escapeHTML(d.first_name),
															siteName : FB.String
																	.escapeHTML(d.site_name)
														}), FB.Intl
												._tx("Learn More"),
										d.profile_url, FB.getDomain('www')
						...
						if (FB.UA.ie()) {
							...
							e.backgroundPosition = '? ' + this._initialHeight
									+ 'px';
...
FB.subclass('XFBML.EdgeWidget', 'XFBML.IframeWidget', null,
		{
			...
			oneTimeSetup : function() {
				this.subscribe('xd.authPrompted', FB.bind(this._onAuthPrompt,
						this));
				this.subscribe('xd.edgeCreated', FB.bind(this._onEdgeCreate,
						this));
				this.subscribe('xd.edgeRemoved', FB.bind(this._onEdgeRemove,
						this));
			...
			_getHeightOffset : function(c) {
				if (c && c.preComputedHeightOffset)
					return parseInt(c.preComputedHeightOffset, 10) + 'px';
			...
			_getCommonEdgeCommentWidgetOpts : function(c, a, d, b) {
				return {
					...
					width : 400,
					height : 300,
					relativeHeightOffset : (b ? this._getHeightOffset(c) : this
							._getHeightOffset()),
					relativeWidthOffset : (d ? this._getWidthOffset(c) : this
							._getWidthOffset()),
					error : c.error,
			...
			_fireEventAndInvokeHandler : function(b, a) {
				...
				FB.Helper.invokeHandler(this.getAttribute(a), this,
						[ this._attr.href ]);
			},
...
FB.subclass('XFBML.Like', 'XFBML.EdgeWidget', null, {
	...
	_createEdgeCommentWidget : function(b, a) {
		if ('send' in this._attr && 'widget_type' in b
				&& b.widget_type == 'send') {
		...
		.subclass(
				...
				null,
				{
					...
					_getLoginText : function() {
						return this._attr.length == 'short' ? FB.Intl
								._tx("Log In") : FB.Intl
								._tx("Log In with Facebook");
					},
					onClick : function() {
						if (!this._attr.registration_url) {
							if (!FB.getAccessToken()
...
FB.subclass('XFBML.Name', 'XFBML.Element', null, {
	process : function() {
		...
		FB.Event.monitor('auth.statusChange', this.bind(function() {
			...
			if (FB.Helper.isUser(this._uid)) {
				...
				a = FB.Data._selectByIndex([ 'name', 'id' ], 'profile', 'id',
						this._uid);
	...
	_renderOther : function(c) {
		...
		if (this._uid == FB.Helper.getLoggedInUser()
			...
			} else if (this._lastnameonly && c.last_name !== undefined)
		...
		.subclass(
				...
				null,
				{
					...
					setupAndValidate : function() {
						...
						function h(k) {
							if (k.match(/^\d+(?:\.\d+)?%$/)) {
								var j = Math.min(Math.max(parseInt(k, 10), 0),
										100);
								k = j / 100;
							} else if (k != 'manual' && k != 'onvisible')
								k = 'onvisible';
							return k;
						}
						function f(j) {
	...
	setupAndValidate : function() {
		...
		if (this._attr.onvalidate)
			this.subscribe('xd.validate', this.bind(function(b) {
				...
				var c = FB.Helper.executeFunctionByName(this._attr.onvalidate,
						d, a);
				if (c)
					a(c);
			}));
		...
		.subclass(
				...
				null,
				{
					...
					_renderButton : function(h) {
						...
						var d = '', e = '', f = '', c = '', g = FB.Intl
								._tx("Share"), i = '';
						switch (this._type) {
						case 'icon':
						case 'icon_link':
							c = 'fb_button_simple';
							d = ('<span class="fb_button_text">'
									+ (this._type == 'icon_link' ? g : '&nbsp;') + '</span>');
							h = false;
							break;
						case 'link':
						...
						case 'button_count':
							d = '<span class="fb_button_text">' + g + '</span>';
							e = ('<span class="fb_share_count_nub_right">&nbsp;</span>'
						...
						default:
							d = '<span class="fb_button_text">' + g + '</span>';
							f = ('<span class="fb_share_count_nub_top">&nbsp;</span>'
					...
					_getCounterMarkup : function() {
						...
						if (this._count.value !== undefined) {
							...
							this._count.wait(FB.bind(this._renderButton, this,
									false));
						return '<span class="fb_share_count_inner">' + b
								+ '</span>';
					}
				});
...
FB.provide("Flash", {
	"_minVersions" : [ [ 10, 0, 22, 87 ], [ 11, 0, 0 ] ],
