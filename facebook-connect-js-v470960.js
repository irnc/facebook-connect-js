/*1321046839,169920896,JIT Construction: v470960,en_US*/
						...
						for ( var e = 0; e < a.length; e++) {
							...
							for ( var f = 0; f < c.childNodes.length; f++)
								if (c.childNodes[f].nodeName.toLowerCase() == "param"
										&& c.childNodes[f].name == "wmode")
									if (c.childNodes[f].value == "opaque"
											|| c.childNodes[f].value == "transparent")
										d = true;
							if (!d) {
								var h = Math.random();
								if (h <= 1 / 1000)
				...
				'Dialog',
				{
					...
					_handleOrientationChange : function() {
						if (FB.UA.iPad()) {
							...
							for ( var a in FB.Dialog._dialogs)
								if (document.getElementById(a))
									document.getElementById(a).style.width = FB.UIServer
		...
		.subclass(
				...
				null,
				{
					...
					_getWidgetWidth : function() {
						...
						var c = (this.getAttribute('action') === 'recommend' ? 130
								: 90)
								+ (g && g !== 'false' ? 60 : 0);
						var b = this.getAttribute('action') === 'recommend' ? 100
								: 55;
						var i = this.getAttribute('action') === 'recommend' ? 90
								: 50;
						var f = {
							standard : {
								show : 450,
					...
					_getHeightOffset : function(c) {
						if (c && c.preComputedHeightOffset)
							return parseInt(c.preComputedHeightOffset, 10)
									+ 'px';
					...
					_getCommonEdgeCommentWidgetOpts : function(c, a, d, b) {
						return {
							...
							width : 400,
							height : 300,
							relativeHeightOffset : (b ? this
									._getHeightOffset(c) : this
									._getHeightOffset()),
							relativeWidthOffset : (d ? this._getWidthOffset(c)
					...
					_getWidthOffset : function(c) {
						if (c && c.preComputedWidthOffset)
							return parseInt(c.preComputedWidthOffset, 10)
									+ 'px';
						var a = this._getLayout();
						var b = {
							standard : '0px',
